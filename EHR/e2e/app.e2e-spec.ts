...
describe('Starting tests for EHR', function() {
  ...
  it('navbar-brand should be EHR',() => {
    ...
    it('Encounter component should be loadable',() => {
      page.navigateTo('/Encounter');
      browser.findElement(by.id('assetName'))
      .then((assetName) => {
        return assetName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('Encounter');
      });
    });

    it('Encounter table should have 2 columns',() => {
      page.navigateTo('/Encounter');
      element.all(by.css('.thead-cols th')).then(function(arr) {
